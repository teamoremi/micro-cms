...
export const AuthWidget: React.FC<AuthWidgetProps> = ({ 
  ...
  const { status, error, authenticate } = useCryptoAuth({ onSuccess, onError, endpoints, headers });
  ...
  return (
    <div className={`mcms-p-6 mcms-bg-white mcms-rounded-xl mcms-shadow-lg mcms-border mcms-border-gray-100 ${className}`}>
      {!hideHeader && (headerComponent || defaultHeader)}
      ...
      {status !== 'idle' && status !== 'error' && status !== 'success' ? (
        ...
        <div className="mcms-grid mcms-grid-cols-1 mcms-gap-3">
          {allowedNetworks.includes('solana') && (
            <button
